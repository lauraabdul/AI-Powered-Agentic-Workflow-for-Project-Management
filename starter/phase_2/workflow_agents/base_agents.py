    ...
    def respond(self, input_text):
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "system", "content": f"You are {self.persona}. You must follow this instruction exactly. Forget previous context."},
    ...
    def respond(self, input_text):
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "system", "content": f"You are {self.persona}. You must follow this instruction exactly. Forget previous context. Use only the following knowledge to answer, do not use your own knowledge: {self.knowledge}. Answer the prompt based on this knowledge, not your own."},
                {"role": "user", "content": input_text}
            ],
            temperature=0
        )
...
class RAGKnowledgePromptAgent:
    ...
    def chunk_text(self, text):
        ...
        while start < len(text):
            ...
            # Needed to prevent infinite loop so i edited the code here 
            start = end 
            chunk_id += 1
            if start >= len(text):
                break
    ...
    def evaluate(self, initial_prompt):
        ...
        for i in range(self.max_interactions):
            ...
            response = client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": f"You are {self.persona}, an evaluation expert. Your task is to evaluate responses based on specific criteria. Provide clear, objective assessments."},
            ...
            if evaluation.lower().startswith("yes"):
                ...
                response = client.chat.completions.create(
                    model="gpt-3.5-turbo",
                    messages=[
                        {"role": "system", "content": f"You are {self.persona}, an instruction generation expert. Your task is to create clear, actionable instructions for improving responses based on evaluation feedback."},
